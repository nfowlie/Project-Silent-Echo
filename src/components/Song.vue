    ...
    <div id="current-artist">
      <div id="bio">
        ...
        <div id="track-list">
          ...
          <div class="track">
            <div class="track-row">
              <div class="full-duration" v-on:click="scrubAudio($event)">
                <div class="current-duration"></div>
              </div>
              <div v-on:click.capture="play(current.tracks.id, current.tracks.length)" class="track-info">
                <div class="track-artist">
                  <div class="track-name">{{ current.tracks.title }}</div>
                  <div class="track-artist-name">{{ current.tracks.album }}</div>
                </div>
                <div class="time-duration">{{ current.tracks.length }}</div>
                <audio :src="current.tracks.source" type="audio/mpeg"></audio>
              </div>
            </div>
            <div class="icons">
            ...
            <div class="icons" v-on:click="share(current.tracks)">
...
export default {
  ...
  created () {
    ...
    this.fillData(this.$route.params.artist, this.$route.params.album, this.$route.params.song)
    this.select(this.$route.params.artist, this.$route.params.album, this.$route.params.song)
  },
  methods: {
    share (track) {
      var clipboard = document.querySelector('.clipboard')
      clipboard.value = 'www.silentechopublishing.com/song/' + this.current.id + '/' + this.current.tracks.album.toLowerCase().replace("'", '') + '/' + track.title
      clipboard.select()
      document.execCommand('copy')
      // this.$router.replace('/song/' + this.current.id + '/' + this.current.tracks[0].album.toLowerCase().replace("'", '') + '/' + track.title)
      alert('Link to ' + track.title + ' by ' + this.current.artist + ' has been copied to your clipboard.')
    },
    play (a, trackDuration) {
      ...
      var pauseButton = document.querySelector('.fa-pause-circle')
      ...
      if (tracks[0].paused) {
        ...
        playTimeTracker = setInterval(function () {
          var remainingTime = parseInt(tracks[0].duration - tracks[0].currentTime, 10)
          var positionTime = (tracks[0].currentTime / tracks[0].duration) * 100
          var mins = Math.floor(remainingTime / 60, 10)
          var secs = remainingTime - mins * 60
          if (secs < 10) {
            secs = '0' + secs
          }
          timeDurations[0].textContent = mins + ':' + secs
          currentDurations[0].style.width = positionTime + '%'
        }, 1000)
      } else {
        tracks[0].pause()
    ...
    scrubAudio (event) {
      var track = document.querySelector('.track-row')
      var skippingTrack = track
